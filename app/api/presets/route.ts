...
export async function POST(request: Request) {
  try {
    ...
    const { name, totalQuestions, startingQuestionNumber, inputType, testMode, timeLimitMinutes, allowOvertime } = body
    ...
    if (startingQuestionNumber === undefined || startingQuestionNumber < 1) {
      return NextResponse.json({ error: "Starting question number must be at least 1" }, { status: 400 })
    }
    ...
    if (testMode === "timer" && (!timeLimitMinutes || timeLimitMinutes < 1)) {
      return NextResponse.json({ error: "Time limit is required for timer-based tests" }, { status: 400 })
    }
    ...
    return NextResponse.json({ error: "Failed to create test preset" }, { status: 500 })
  }
}
...
export async function GET() {
  try {
    ...
    return NextResponse.json({ error: "Failed to fetch test presets" }, { status: 500 })
  }
}
