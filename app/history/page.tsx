...
import { ArrowLeft, Clock, CheckCircle, XCircle, FileText, Eye, Timer } from "lucide-react"
    ...
    <div className="min-h-screen bg-background">
      <header className="border-b border-border bg-card">
        <div className="mx-auto max-w-7xl px-4 py-6">
          ...
          <p className="mt-1 text-sm text-muted-foreground">View all your past test attempts</p>
        </div>
      </header>
        ...
        {history.length === 0 ? (
          <Card>
            <CardContent className="flex flex-col items-center justify-center py-12">
              ...
              <p className="mb-6 text-sm text-muted-foreground">Take a test to see your history here</p>
              <Link href="/presets">
                  ...
                  <TableBody>
                    {history.map((attempt) => (
                      <TableRow key={attempt.id}>
                        <TableCell className="font-medium">{attempt.preset_name}</TableCell>
                        <TableCell>
                        ...
                        <TableCell>
                          {attempt.is_evaluated && attempt.percentage !== null ? (
                            <span className="text-lg font-bold">{attempt.percentage.toFixed(1)}%</span>
                          ) : (
                        ...
                        <TableCell className="text-center">
                          {attempt.is_evaluated ? (
                            <span className="text-muted-foreground">{attempt.unanswered}</span>
                          ) : (
                            <span className="text-muted-foreground">-</span>
                          )}
                        </TableCell>
                        <TableCell>
                          <div className="flex items-center gap-1">
                            ...
                            {attempt.overtime_seconds && attempt.overtime_seconds > 0 && (
                              <span className="text-xs text-destructive">+{formatTime(attempt.overtime_seconds)}</span>
                            )}
                          </div>
                        </TableCell>
