    ...
    <Card>
      ...
      <CardContent className="space-y-6">
        {Object.entries(testStats).map(([testId, stats]) => {
          const avgScore = stats.attempts.reduce((sum, a) => sum + Number(a.percentage), 0) / stats.attempts.length
          const bestScore = Math.max(...stats.attempts.map((a) => Number(a.percentage)))
          const totalCorrect = stats.attempts.reduce((sum, a) => sum + a.correct_answers, 0)
          const totalQuestions = stats.attempts.reduce((sum, a) => sum + a.total_questions, 0)
            ...
            <div key={testId} className="space-y-2">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">{stats.attempts.length} attempts</p>
                </div>
                <div className="text-right">
                  <p className="text-sm font-medium">Avg: {avgScore.toFixed(1)}%</p>
                  <p className="text-xs text-muted-foreground">Best: {bestScore.toFixed(1)}%</p>
                </div>
              </div>
