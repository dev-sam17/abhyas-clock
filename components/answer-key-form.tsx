...
export function AnswerKeyForm({ attempt }: { attempt: Attempt }) {
  ...
  const [correctAnswers, setCorrectAnswers] = useState<(string | null)[]>(Array(attempt.totalQuestions).fill(null))
  ...
  const handleSubmit = async () => {
    const unansweredKeys = correctAnswers
      .map((ans, idx) => (ans === null ? attempt.preset.startingQuestion + idx : null))
      .filter((num) => num !== null)

    if (unansweredKeys.length > 0) {
      toast.error(`Please enter correct answers for all questions. Missing: ${unansweredKeys.join(", ")}`)
      return
    }
  ...
  const getUserAnswer = (questionNumber: number) => {
    const answer = attempt.answers.find((ans) => ans.questionNumber === questionNumber)
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Enter the correct answer for each question. This will be used to calculate your score.
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid gap-4">
            {Array.from({ length: attempt.totalQuestions }, (_, i) => attempt.preset.startingQuestion + i).map(
              (questionNumber) => {
                const userAnswer = getUserAnswer(questionNumber)
                const correctAnswer = correctAnswers[questionNumber - attempt.preset.startingQuestion]
                    ...
                    {attempt.preset.inputType === "radio" ? (
                          ...
                          <div className="flex gap-2">
                            {["A", "B", "C", "D", "E"].map((option) => (
                              <button
                                key={option}
                                type="button"
                                onClick={() => selectCorrectAnswer(questionNumber, option)}
                                className={cn(
                      ...
                      <div className="flex-1">
                        ...
                        <Input
                          ...
                          value={correctAnswer || ""}
                          onChange={(e) => setTextCorrectAnswer(questionNumber, e.target.value)}
                          className="h-10"
                        />
                      </div>
                    )}
        ...
        <CardContent className="pt-6">
          ...
          {filledCount < attempt.totalQuestions && (
            <p className="text-sm text-muted-foreground text-center mt-2">
              Please enter correct answers for all {attempt.totalQuestions} questions
            </p>
          )}
        </CardContent>
      </Card>
